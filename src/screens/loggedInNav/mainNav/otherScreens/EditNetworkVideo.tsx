...
const EditNetworkVideo = ({navigation,route}) => {
  ...
  const trimVideoWithSaveOptionReturnAlsoPureVideoFileAndGivenParamForIOSThumbNailUri = (thumbNail) => {
    const options = {
  ...
  const onPressEditComplete = () => {
    ...
    getPreviewImageForIOSThumbnail(nowTrim.startTime)
      .then(thumbNail => trimVideoWithSaveOptionReturnAlsoPureVideoFileAndGivenParamForIOSThumbNailUri(thumbNail))
      .then(completeResult => navigation.navigate("EditPost",completeResult))
      .catch(ifErrorThenAlert);
  };
