...
import { CopiedFileInfo, FileInfo } from "../../../../components/petLog/type/fileType";
import useMaterialTabGetInnerLayoutHeight from "../../../../hooks/useMaterialTabGetInnerLayoutHeight";

const EDIT_PETLOG = gql`
  mutation editPetLog(
    $id:Int!
    $title: String,
    $body: [String],
...
// UI 는 UploadPetLog, 로직은 EditPost 랑 거의 비슷
const EditPetLog = ({navigation,route}) => {
  ...
  const routeParams:EditPetLogProps = route.params;
  // const [petLogId,setPetLogId] = useState(routeParams.petLogId);
  ...
  const petLogId = useRef(routeParams.petLogId).current;
  ...
  const updateEditPetLog = (cache,result) => {
    const {data:{editPetLog:{ok,error}}} = result;
  ...
  const [editPetLog,{loading}] = useMutation(EDIT_PETLOG);
  ...
  useEffect(()=>{
    ...
    navigation.setOptions({
      ...
      headerLeft:({tintColor}) => <UploadGoBackBtn tintColor={tintColor} whichComponent="EditPetLog" alertCheck={isSomethingWrite} />
    });
  ...
  // UploadPetLog 와 다르게 여기서만 필요해서 여기서 state 로 씀
