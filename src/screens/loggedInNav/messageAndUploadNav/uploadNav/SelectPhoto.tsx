...
const SelectPhoto = ({navigation,route}:Props) => {
  ...
  const HeaderRight = () => <TouchableOpacity onPress={async() => {
    ...
    if(chosenPhotoIdArr.length === 0){
      ...
      // EditPetLog 에서 온거면 PetLogUploadForm 로 이동
      isEditPetLog ?
  ...
  // download 폴더에 있는 이미지가 안나옴. 걍 ImagePicker?
