...
import { Alert, Keyboard, Platform, TextInput, TouchableOpacity } from "react-native";
import styled from "styled-components/native";
import AuthButton from "../../components/auth/AuthButton";
import AuthLayout from "../../components/auth/AuthLayout";
import Input from "../../components/auth/AuthShared";
import { emailCheck, passwordCheck, userNameCheck, birthCheck, realNameCheck } from "../../logic/userInfoFormCheck";
...
import GenderChooseBtn from "../../components/auth/GenderChooseBtn";
...
`;
const GenderGuideText = styled.Text<{placeholderTextColor:string,isAndroid:boolean}>`
  margin-left: ${props=>props.isAndroid ? "10px" : "8px"};
...
const AgreeContainer = styled.View`
  ...
  align-items: center;
`;
const IsNotCheckAgreeBaseText = styled.Text<{isAndroid:boolean}>`
  color: tomato;
...
`;
const AgreeLinkText = styled.Text<{isAndroid:boolean,isDarkMode:boolean}>`
...
const CreateAccount = ({navigation:{navigate,replace}}:Props) => {
  ...
  const disabled = loading || createAccountResultOk;
  ...
  return <AuthLayout>
    <Controller
      ...
      name="email"
    />
    {errors.email?.type === "required" && <AlertText>이메일은 필수 항목 입니다.</AlertText>}
    {errors.email?.type === "validate" && <AlertText>이메일 형식이 맞지 않습니다.</AlertText>}
    {/* 이메일 겹칠때 보여주는거. shouldUnregister 는 상관없음 내가 setError 로 지정함. */}
    {errors.email?.type === "shouldUnregister" && <AlertText>같은 이메일의 계정이 존재합니다.</AlertText>}
    <Controller
      control={control}
      rules={{
        required: true,
      ...
      name="password"
    />
    {errors.password?.type === "required" && <AlertText>비밀번호를 작성해 주세요.</AlertText>}
    {errors.password?.type === "validate" && <AlertText>비밀번호 형식은 숫자+영문+특수문자 조합의 8~25 자 이어야 합니다.</AlertText>}
    <Controller
      control={control}
      rules={{
        required: true,
      ...
      name="userName"
    />
    {errors.userName?.type === "required" && <AlertText>닉네임은 필수 항목 입니다.</AlertText>}
    {errors.userName?.type === "validate" && <AlertText>닉네임에는 20자 이하의 영어, 한글, 숫자만 사용 가능합니다.</AlertText>}
    {/* 닉네임 겹칠때 보여주는거. shouldUnregister 는 상관없음 내가 setError 로 지정함. */}
    {errors.userName?.type === "shouldUnregister" && <AlertText>같은 닉네임의 계정이 존재합니다.</AlertText>}
    <Controller
      control={control}
      rules={{
        // 이름규정?
    ...
    <AgreeContainer>
      {isNotCheckAgree && <><IsNotCheckAgreeText isAndroid={isAndroid} >!</IsNotCheckAgreeText>
      <IsNotCheckAgreeMark isAndroid={isAndroid} >필수</IsNotCheckAgreeMark></>
      }
      ...
      <AgreeText>쏘큐티의 </AgreeText>
      <AgreeLink onPress={onPressTermsOfService} >
        <AgreeLinkText isAndroid={isAndroid} isDarkMode={isDarkMode} >이용약관</AgreeLinkText>
      </AgreeLink>
      <AgreeText>, </AgreeText>
      <AgreeLink onPress={onPressPrivacy} >
        <AgreeLinkText isAndroid={isAndroid} isDarkMode={isDarkMode} >개인정보처리방침</AgreeLinkText>
      </AgreeLink>
      <AgreeText> 에 동의합니다.</AgreeText>
    </AgreeContainer>
